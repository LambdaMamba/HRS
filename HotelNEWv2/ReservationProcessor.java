...
public class ReservationProcessor{
    ...
    private static int NumberOfRooms = 20;
    ...
    public static void CheckHowManyWantToReserveRoom(){
        ...
        if(NumberOfAvailableRooms<1){
            ...
            System.out.println("[Processor]: # of users who want to reserve: " + NumberOfOnlineReservationInstances);
            userIsChosen = RandomlySelectOneUser(NumberOfOnlineReservationInstances);
            if(userIsChosen == true){
                System.out.println("[Processor]: User is chosen");
                OnlineReservation.UserHereIsSelected();
                SelectedID = tempID;
            } else if (userIsChosen == false){
                System.out.println("[Processor]: User is not chosen");
                OnlineReservation.UserHereIsNotSelected();
                AddToWaitingList(tempID);
            }
    ...
    public static boolean RandomlySelectOneUser(int NumberOfOnlineReservationInstances){
        ...
        System.out.println("[Processor] Dice: "+dice);
        if(dice == 1){
            return true;
        }else{
            return false;
    ...
    public static void AddToWaitingList(int userID){
        ...
        System.out.println("[Processor] Current waiting list queue: "+WaitingList);
        System.exit(0);
    }
    ...
    public static void RegisterReservation(boolean Wifi, String Date){
        ...
        //Pretend staff
        // ReservationProcessor.StartWifi(2, Wifi);

    }

    public static void UpdateAvailableRoomList(){
        int i = 0;
        System.out.println("[Processor] Old Available room list" +Arrays.toString(AvailableRoomList));
        while(true){
            if(AvailableRoomList[i] != -1){
                RoomForReservation = AvailableRoomList[i];
                AvailableRoomList[i] = -1;
                System.out.println("[Processor] RoomForReservation: "+ RoomForReservation);
                System.out.println("[Processor] New Available room list" +Arrays.toString(AvailableRoomList));
                ReservationDictionary.put(ReservationNumber, RoomForReservation);

                System.out.println("[Processor] Room number given to this reservation number: " + ReservationDictionary.get(ReservationNumber));
    ...
    public static int SearchFeeOfRoomNumber(int RoomNumber){
        ...
        if(wifitruth==true){
            ...
            System.out.println("[Processor] Wifi Usage (Minuites): "+WifiUsage);
        ...
        System.out.println("[Processor] Base Fee: "+basefee);
